...
- adapter files are refreshed only when recognized as generated by this script
...
2. Creates or appends missing vendor instruction sections in `AGENTS.md`.
...
5. Refreshes generated adapter shims when `--adapter-mode shim` is active.
...
Currently, `write_shims()` only manages a generated `CLAUDE.md` adapter. If an
existing `CLAUDE.md` lacks the generated marker, the script preserves it and
reports `preserve existing CLAUDE.md`.
